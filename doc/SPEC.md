...
| 枠 | 生成条件 | 理由 |
|---|---|---|
| 宿 | 泊数分（連泊は1枠にまとめる） | 最大の支出、早期予約が必須 |
| 往路の移動 | 常に | 早期予約で価格が大きく変わる |
| 復路の移動 | 常に（日帰りなら往復1枠） | 同上 |

2泊3日なら4枠のみ。食事もアクティビティも自動生成しない。
...
- **連泊は1枠にまとめる** — デフォルトは「宿・2泊」で1枠。金額入力は「1泊あたり／合計」をトグルで選択。途中で宿を変える場合は「この枠を分割」で2枠に割る（結合は使用頻度が低くMVPでは省略）
...
1. 旅行作成（日程・目的地・人数）→ 宿・往路・復路の枠を自動生成
2. 概算プレースホルダと締切の自動設定
3. 枠の手動追加・削除・並べ替え
