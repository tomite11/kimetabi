...
コード、設定、DBマイグレーションを実装・変更した場合は、完了報告の前に必ず関連テストを実行し、`code-review-excellence` を使って変更差分をコードレビューしてください。レビューで指摘された重大・高優先度の問題は修正し、影響するテストを再実行します。テスト失敗、未解決の重大指摘、テストまたはレビュー未実施の状態を完了扱いにしてはいけません。完了報告には、実行したテスト、結果、レビュー結果、未解決事項を記載してください。
